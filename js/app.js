...
const App = {
  ...
  steps() {
    return [
      ...
      () => `<div class="ob-hero">
        ...
        <h1>Oracle</h1>
        <p class="ob-tag">Ton compagnon de <b>jeu de rôle</b> — un co-Maître du Jeu IA qui s'adapte à <b>tous les univers</b>, mène la partie en temps réel et retient toute ta campagne.</p>
      ...
      () => `<div class="ob-panel">
        <h2>Quel univers ?</h2>
        <p class="ob-sub">Tu pourras tout changer ensuite. L'Oracle s'adaptera au thème choisi.</p>
  ...
  obPickGenre(k) {
    ...
    this.ob.draft.attrs = g.attrs.slice(); this.ob.draft.system = g.system; this.ob.draft.theme = g.theme;
  ...
  obHeroAdd() { this.ob.heroes.push({ name: "", player: "" }); this.renderOb(); },
  ...
  obFinish() {
    ...
    this.ob.heroes.filter(h => h.name.trim()).forEach(h => {
      State.addHero({ name: h.name.trim(), player: h.player.trim(),
